...
def fetch_city_info_fallback(location):
    ...
    response = requests.get(
        ...
        headers={**HEADERS, "User-Agent": "xiaozhi-esp32-server/1.0"},
        timeout=10,
    )
    ...
    def score(item):
        ...
        if addresstype == "city":
            type_score = 4
        elif addresstype in {"town", "county"}:
            type_score = 3
        elif addresstype in {"state_district", "province"}:
            type_score = 2
        location_score = 1 if location in display_name else 0
        china_score = 1 if "中国" in display_name else 0
        return (type_score, location_score, china_score, importance)
...
def fetch_weather_report_fallback(location):
    city_info = fetch_city_info_fallback(location)
    if not city_info:
        return None
    ...
    current_weather = OPEN_METEO_CODE_MAP.get(current.get("weather_code"), "未知")
    weather_report = f"您查询的位置是：{city_info['display_name']}\n\n当前天气: {current_weather}\n"
    weather_report += "详细参数：\n"
    weather_report += f"  · 温度: {current.get('temperature_2m', '未知')}°C\n"
    weather_report += f"  · 体感温度: {current.get('apparent_temperature', '未知')}°C\n"
    weather_report += f"  · 相对湿度: {current.get('relative_humidity_2m', '未知')}%\n"
    weather_report += f"  · 降水: {current.get('precipitation', '未知')}mm\n"
    weather_report += f"  · 风速: {current.get('wind_speed_10m', '未知')}km/h\n"
    ...
    for date, code, high, low in zip(times, codes, max_temps, min_temps):
        weather = OPEN_METEO_CODE_MAP.get(code, "未知")
...
def get_weather(conn: "ConnectionHandler", location: str = None, lang: str = "zh_CN"):
    ...
    if not location:
        return ActionResponse(
            Action.REQLLM,
            "我还不知道你所在的城市，请直接告诉我城市名，比如“杭州天气怎么样”。",
            None,
        )
    # 尝试从缓存获取完整天气报告
    ...
    if not city_info:
        try:
            fallback_report = fetch_weather_report_fallback(location)
        except Exception as e:
            logger.bind(tag=TAG).warning(f"公开天气接口兜底失败: {e}")
